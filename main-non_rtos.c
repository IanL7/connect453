...
int main(void)
{
	...
	if(ALERT_CONSOLE_RX){
		if(strcmp(inputString,"temp")==0){
			temp = LM75_get_temp();
			printf("\r\nThe temperature is: %.2f\r\n", temp);
		}
		else if(strstr(inputString, "led off") != NULL){
			int num = inputString[8]-48;
			if(num<0 || num>6){
				num = 10;
		...
		else if(strstr(inputString, "led on") != NULL){

			int num = inputString[7]-48;
			if(num<0 || num>6){
				num = 10;
				printf("\r\nInvalid command, please follow the format: led on <0-6>\r\n");
			}
			...
			printf("Invalid command, please enter either \"temp\" or \"led [on/off] [0-6] \r\n");
		}
